...
class LAHelper
{
	...
	public static function print_menu($menu, $active = false) {
        ...
        if($menu->name == 'Employees') {
            $str = '<li'.$treeview.' '.$active_str.'><a href="'.url(config("laraadmin.adminRoute") . '/' . $menu->url ) .'" title="Master upload file manager"><i class="fa '.$menu->icon.'"></i> <span>'.LAHelper::real_module_name('User').'</span> '.$subviewSign.'</a>';
        } else if($menu->name == 'Uploads') {
            $str = '<li'.$treeview.' '.$active_str.'><a href="'.url(config("laraadmin.adminRoute") . '/' . $menu->url ) .'" title="Master upload file manager"><i class="fa '.$menu->icon.'"></i> <span>'.LAHelper::real_module_name($menu->name).'</span> '.$subviewSign.'</a>';
        } else {
